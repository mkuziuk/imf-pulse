...
@pytest.mark.parametrize("mutation_target", ["bound", "live"])
def test_post_gate_mutation_preserves_checkpoint_and_rolls_back_binding(
    tmp_path: Path,
    empty_knowledge: Path,
    schemas_directory: Path,
    mutation_target: str,
    ...
    config, first = _prepare_candidate(
        project, source, empty_knowledge, schemas_directory, "version one"
    )
    publish_release(
        project,
        first.release_id,
    ...
    (source / "README.md").write_text("version two", encoding="utf-8")
    ...
    pulse, manifest_url, live_svg = _write_publication(project, second.release_directory)

    def mutate(command, cwd, environment):
        if tuple(command) == ("npm", "test"):
            target = (
                second.release_directory
                / "publication"
                / "public"
                / manifest_url.removeprefix("/").rsplit("/", 1)[0]
                / "chart.svg"
                if mutation_target == "bound"
                else live_svg
            )
            target.write_text("<svg>mutated</svg>\n", encoding="utf-8")
        materialize_test_site(command, environment)
    ...
    assert not (second.release_directory / "publication").exists()
    assert "publication" not in read_json(second.release_directory / "release.json")
    validate_release_directory(second.release_directory, schemas_directory)
...
def test_no_pulse_run_retains_immutable_accepted_history(
    tmp_path: Path, empty_knowledge: Path, schemas_directory: Path
) -> None:
    project = tmp_path / "project"
    source = tmp_path / "source"
    config, first = _prepare_candidate(
        project, source, empty_knowledge, schemas_directory, "day one"
    )
    pulse, manifest_url, _ = _write_publication(project, first.release_directory)
    publish_release(
        project,
        first.release_id,
        schemas_directory=schemas_directory,
    ...
    (source / "README.md").write_text("day two knowledge only", encoding="utf-8")
    _, snapshot_directory, _ = build_snapshot(config, project)
    second = build_release_candidate(
        project,
        config,
    ...
    def gate(command, cwd, environment):
        contexts.append(dict(environment))
        materialize_test_site(command, environment)
    ...
    assert all("IMF_PULSE_SELECTED_PULSE" not in context for context in contexts)
    assert all(
        json.loads(context["IMF_PULSE_ARTIFACT_MANIFESTS"]) == []
        for context in contexts
    )
    assert all(
        context["IMF_PULSE_CHECKPOINT_STATUS"] == "processed_no_pulse"
        for context in contexts
    )
    pointer = read_json(project / "data" / "current.json")
    assert pointer["pulse"] is None
    assert pointer["artifact_manifests"] == []
    assert pointer["latest_accepted_pulse"] == pulse
    assert pointer["accepted_pulses"] == [pulse]
    assert pointer["accepted_publications"][0]["release_id"] == first.release_id
    assert pointer["site_build_path"] == (
        f"data/site-builds/site-{pointer['site_build_sha256']}"
    )


def test_successor_release_uses_bound_history_after_live_publication_inputs_change(
    tmp_path: Path, empty_knowledge: Path, schemas_directory: Path
) -> None:
    project = tmp_path / "project"
    source = tmp_path / "source"
    ...
    accepted_before = read_json(project / "data" / "current.json")[
        "accepted_publications"
    ]
    bound_manifest = first.release_directory / accepted_before[0][
        "artifact_manifests"
    ][0]["bound_path"].removeprefix(f"data/releases/{first.release_id}/")
    bound_manifest_bytes = bound_manifest.read_bytes()

    live_manifest = project / "public" / manifest_url.removeprefix("/")
    changed_manifest = read_json(live_manifest)
    changed_manifest["caption"] = "A later editorial revision of the live artifact."
    live_manifest.write_text(
        json.dumps(changed_manifest, sort_keys=True) + "\n", encoding="utf-8"
    )
    (project / "scripts" / "generate_test.py").write_text(
        "# later deterministic generator revision\n", encoding="utf-8"
    )

    # Even an unchanged retry builds the accepted archive from its immutable
    # binding rather than from a mutable working-copy path.
    unchanged = publish_release(
        project,
        first.release_id,
        schemas_directory=schemas_directory,
        gate_runner=_ok_gate,
    ...
    assert pointer["accepted_publications"] == accepted_before
    assert bound_manifest.read_bytes() == bound_manifest_bytes
    verify_bound_publication(
...
def test_sealed_history_digest_rejects_archive_truncation_or_reordering(
    tmp_path: Path,
    empty_knowledge: Path,
    schemas_directory: Path,
    tamper: str,
    ...
    (source / "README.md").write_text("day two", encoding="utf-8")
    _, snapshot_directory, _ = build_snapshot(config, project)
    second = build_release_candidate(
        project,
        config,
    ...
    accepted = pointer["accepted_publications"]
    assert len(accepted) == 2
    tampered = accepted[:1] if tamper == "truncate" else list(reversed(accepted))
    pointer["accepted_publications"] = tampered
    pointer["accepted_publications_sha256"] = canonical_json_hash(tampered)
    pointer["accepted_pulses"] = [item["pulse"] for item in tampered]
    pointer["latest_accepted_pulse"] = tampered[-1]["pulse"]
    artifact_urls: list[str] = []
    for publication in tampered:
        for artifact in publication["artifact_manifests"]:
            if artifact["url"] not in artifact_urls:
                artifact_urls.append(artifact["url"])
    pointer["accepted_artifact_manifests"] = artifact_urls
    pointer["latest_accepted_artifact_manifests"] = [
        artifact["url"] for artifact in tampered[-1]["artifact_manifests"]
    ]
    pointer_path.write_text(json.dumps(pointer, sort_keys=True) + "\n", encoding="utf-8")
    tampered_checkpoint = pointer_path.read_bytes()
    with pytest.raises(PublicationError, match="sealed release digest"):
        publish_release(
            project,
            second.release_id,
            schemas_directory=schemas_directory,
    ...
    assert pointer_path.read_bytes() == tampered_checkpoint
...
def test_later_release_rejects_an_already_accepted_identical_pulse(
    tmp_path: Path, empty_knowledge: Path, schemas_directory: Path
) -> None:
    project = tmp_path / "project"
    source = tmp_path / "source"
    config, first = _prepare_candidate(
        project, source, empty_knowledge, schemas_directory, "first release"
    )
    pulse, manifest_url, _ = _write_publication(project, first.release_directory)
    publish_release(
        project,
    ...
    (source / "README.md").write_text("changed release bytes", encoding="utf-8")
    _, snapshot_directory, _ = build_snapshot(config, project)
    second = build_release_candidate(
        project,
        config,
    ...
    with pytest.raises(PublicationError, match="accepted pulse cannot be published"):
        publish_release(
            project,
            second.release_id,
            schemas_directory=schemas_directory,
