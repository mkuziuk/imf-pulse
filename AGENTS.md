...
- External monitoring is metadata-only and allowlisted to arXiv and Crossref. The Luna scout may rank only hash-bound metadata candidates in private storage and is never evidence or publication authority. Aegis must verify the immutable Luna inbox, reject instruction-like or non-allowlisted material, fetch exact-hash-bound arXiv PDFs through the reviewed evidence helper, and statically parse them before sealing an approved bundle. Sol has no network or shell access and may use only that approved bundle.
...
- Unresolved metadata is deferred and never blocks the day. Automatic publication requires a fail-closed offline package from GPT-5.6 Sol, an Aegis approval bound to the exact candidate and PDF hashes, page-level evidence, append-only knowledge, deterministic novelty selection, and all publication gates. Unknown source-media reuse rights do not veto original cited analysis; they forbid republication of that media and require a ChatGPT-generated raster conceptual surrogate with original visual form when a replacement visual is warranted. Scheduled automatic reports do not use diagrams. Evidence, identity, novelty, scope, page-support, or public-visual provenance uncertainty is skipped.
...
- A deterministic host-side publisher runs `scripts/run_scheduled_pipeline.py` only after the dated Aegis bundle and offline Sol package are both present. That wrapper must run the daily transaction exactly once.
